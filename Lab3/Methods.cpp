...
double GradientDescentMethod::GetMin() const
{
    ...
    // Вывод результатов
    std::cout << "Минимум достигается в: (" << x << ", " << y << ")" << std::endl;
    std::cout << "Значение функции в этой точке: " << f(x, y) << std::endl;
   ...
   double eps) const
{
   ...
   // Основная идея: искать минимум функции g(alpha) с учетом заданной точности
    ...
    double alpha_max = (dx > 0 || dy > 0) ? -1 : 1; // предполагаемый максимальный диапазон для alpha
...
FastestGradientDescentMethod::GetMin() const
{
   ...
   std::cout << "Минимум достигается в: (" << x << ", " << y << ")\n";
   std::cout << "Значение функции в этой точке: " << f(x, y) << "\n";
